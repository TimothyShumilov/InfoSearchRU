"""
Evaluate Qwen/Qwen3-Embedding-4B on the InfoSearch benchmark tasks
...
# Reuse utils and model_config from the qwen3_06b directory
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "qwen3_06b"))
...
# ── CLI ───────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Evaluate Qwen3-Embedding-4B on InfoSearch tasks")
parser.add_argument("--model_name_or_path", default="Qwen/Qwen3-Embedding-4B",
                    type=str, help="Model identifier or local path")
parser.add_argument("--output_dir", default="qwen3-embedding-4b-infosearch",
                    type=str, help="Directory for MTEB result JSONs")
parser.add_argument("--batch_size", default=8, type=int, help="Encoding batch size")
...
class RetrievalModel(DRESModel):
    ...
    def _do_encode(self, input_texts: List[str]) -> np.ndarray:
        encoded_embeds = []
        batch_size = args.batch_size * max(self.gpu_count, 1)
